...
let docker_build_str ~debug ~conf ~max_ram_per_job ~base_dockerfile ~stderr ~default c =
  let rec aux ~stdin =
    ...
    match Oca_lib.read_line_opt stdin with
    | Some "@@@OUTPUT" ->
        let rec aux acc =
          match Oca_lib.read_line_opt stdin with
        ...
        aux []
    | Some line ->
        let () = if debug then Oca_lib.write_line stderr line else () in
        aux ~stdin
    | None -> []
  in
  match
...
let update_docker_image conf =
  ...
  | [] -> assert false
  | [image] ->
      begin match get_latest_image ~image with
...
let get_max_ram_per_job ~number_of_jobs =
  (* in Megabytes *)
  let free_ram = (ExtUnix.All.sysinfo ()).ExtUnix.All.freeram / 1_000_000 in
  let free_ram_per_job = free_ram / number_of_jobs in
  string_of_int free_ram_per_job ^ "m"
